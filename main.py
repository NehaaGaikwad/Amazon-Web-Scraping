...
def render_product_card(product):
    with st.container(border=True):
        ...
        with cols[1]:
            ...
            if currency.upper() == "USD" and price:
                price_inr = convert_usd_to_inr(price)
                price_display = f"₹{price_inr}  ($ {price})" if price_inr else f"$ {price}"
            elif price:
                price_display = f"{currency} {price}"
            else:
                price_display = "N/A"

            info_cols[0].metric("Price", price_display)
